...
def general_forward_SDE_ps(
    ...
    if inference_snr_db is not None:
        ...
        #gt_img = mri_inf_utils.adjoint(y)  # Get image domain from k-space measurements
        #s_mag_sq = torch.abs(gt_img)**2
        #P = torch.mean(s_mag_sq).item() # L=1 assumed
        adj_img = mri_inf_utils.adjoint(y)
        s_mag_sq = torch.abs(adj_img)**2
        P_meas = torch.mean(s_mag_sq).item()
        
        if inference_snr_db == 15:
            decresed_snr_db = inference_snr_db -3  # Reduce SNR by 3 dB to account for measurement noise
        else:
            decresed_snr_db = inference_snr_db
        gamma = 10**(-float(decresed_snr_db)/10)
        
        # 4. Estimate Clean Signal Power (P_est)
        # Algebra: P_meas = P_est * (1 + gamma)
        #P_est = P_meas / (1 + gamma)
    ...
    for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])): # 0, ..., N-1
        ...
        # Euler step on Prior.
        h = t_next - t_hat
        network_input = x_hat / s(t_hat)
        # Concatenate along channel dim (dim=1) -> [B, 4, H, W]
        network_input = torch.cat([network_input, prior_in_channels], dim=1)
        # Run Net
        denoised_all = net(network_input, sigma(t_hat), class_labels).to(torch.float64)
        # Slice output (keep only image channels)
        denoised = denoised_all[:, 0:2, :, :]
        """
        network_input = torch.stack([network_input.unsqueeze(1), img_prior.unsqueeze(1)], dim=1)
        denoised = net(network_input, sigma(t_hat), class_labels).to(torch.float64)
        ...
        #if i <= num_steps - 150 : 
        if current_sigma > sigma_input_threshold:
        #if i<= 350:
