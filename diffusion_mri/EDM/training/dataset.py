...
class NoisyFolderDataset(Dataset):
    def __init__(self,
        ...
        **super_kwargs,          # Additional arguments for the Dataset base class.
        ...
        # --- FIXED SECTION: REMOVED u_images LOGIC ---
        # for i in range(len(self._data["x_est"])):
        ...
        # --- ROBUST NORMALIZATION FIX (Complex Safe) ---
        for i in range(len(self._data["x_est"])):
            img = self._data["x_est"][i]
            
            # 1. Calculate Magnitude for the quantile (Safe)
            mag = torch.abs(img) 
            robust_max = torch.quantile(mag, 0.99)
            ...
            # 4. Clamp (FIXED: Clamp Real/Imag separately)
            # We cannot clamp complex directly, so we split them
            real_clamped = img.real.clamp(-1.5, 1.5)
            imag_clamped = img.imag.clamp(-1.5, 1.5)
            
            # Recombine and save
            self._data["x_est"][i] = torch.complex(real_clamped, imag_clamped)


        self._type = 'memory'
        # count = len(self._data["u_images"]) # This caused the keyerror
        count = len(self._data["x_est"]) # Use x_est instead, it always exists
        ...
        raw_shape = [len(self._image_fnames)] + list(self._load_raw_image(0).shape)
        raw_shape[1] = 2 # This hardcodes 2 channels (Real, Imag)
        print("Dataset Shape: " + str(raw_shape) + "\n")
        super().__init__(name=name, raw_shape=raw_shape, **super_kwargs)
    ...
    def _load_raw_image(self, raw_idx):
        idx = int(self._image_fnames[raw_idx].split(".")[0])
        # --- FIXED SECTION: REMOVED u_images ---
        ...
        ####### Edition for DPS-PI #########
        #x_est_prior = np.array(torch.view_as_real(self._data["x_est_prior"][idx]).permute(2, 0, 1))
        # concat 4 channels: Real_noisy, Imag_noisy, Real_prior, Imag_prior
        #x_est = np.concatenate([x_est, x_est_prior], axis=0)
        ####### Edition for DPS-PI #########

        # Return ONLY the noisy image (2 channels: Real, Imag)
        return x_est 
